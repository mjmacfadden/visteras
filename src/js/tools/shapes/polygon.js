...
import { create_polygon_subpath } from './../../core/vector/vector-shapes.js';
...
class Polygon_class extends Base_tools_class {
	...
	mousedown(e) {
		...
		const sides = Number(params.sides?.value ?? params.sides ?? params.corners?.value ?? params.corners ?? 5);

		const subpath = create_polygon_subpath(mouse_x, mouse_y, 0, 0, sides);
		const vectorCount = (config.vectors ? config.vectors.length : 0) + 1;

		const vec = new Vector({
			name: 'Polygon ' + vectorCount,
	...
	mousemove(e) {
		...
		const sides = Number(params.corners?.value ?? params.corners ?? params.sides ?? 5);
		const subpath = create_polygon_subpath(x, y, width, height, sides);
	...
	mouseup(e) {
		...
		// If click without drag, create a default 100x100 polygon
		if (width < 2 && height < 2) {
			x = click_x - 50;
			y = click_y - 50;
			width = 100;
		...
		app.State.do_action(
			new Modify_path_action(vec.id, vec.paths, 'Create Polygon', {
	...
	demo(ctx, x, y, width, height) {
		const subpath = create_polygon_subpath(x, y, width, height, 5);
		ctx.beginPath();
		for (let i = 0; i < subpath.anchors.length; i++) {
			const pt = subpath.anchors[i].point;
